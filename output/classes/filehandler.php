...
class RunnerFileHandler {
	...
	protected function validateFile( $filename, $fileSize, &$errorMessage ) {
        if ( !$filename ) {
            $errorMessage = "Aucun nom de fichier n'était indiqué";
            return false;
        }
		...
		if( $acceptedTypes ) {
			...
			if( array_search( $ext, $acceptedTypes ) === false && array_search( ".".$ext, $acceptedTypes ) === false ) {
				$errorMessage = "Type de fichier inacceptable";
				return false;
			}
		}
		...
		if ( $maxFileSize && $fileSize > $maxFileSize * 1024 ) {
            $errorMessage = mysprintf("Largeur du fichier excède la limite de %s kilo-octets", array( $maxFileSize ) );
            return false;
        }
		...
		if ( $maxTotalFileSize ) {
			if( $this->getUploadFilesSize() + $fileSize > $maxTotalFileSize * 1024 ) {
				$errorMessage = mysprintf( "Largeur total des fichiers excède la limite de %s kilo-octets", array( $maxTotalFileSize ) );
				return false;
			}
		}
		...
		if ( $maxNumberOfFiles && $this->getUploadFilesCount() >= $maxNumberOfFiles ) {
			$errorMessage = mysprintf( "Vous ne pouvez télécharger que %s fichiers", array( $maxNumberOfFiles ) );
			return false;
		}
        return true;
	}
