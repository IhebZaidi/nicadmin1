...
class RunnerPage
{
	...
	function assignBodyEnd($params = "")
	{
		...
		echo "<script language=\"JavaScript\" src=\"".GetRootPathForResources("include/runnerJS/RunnerAll.js?41053")."\"></script>\r\n";
	...
	function buildPaginationControl( $currentPage, $maxPages, $limit ) {
		...
		if($counterstart != 1)
		{
			$pagination.= $this->getPaginationLink(1,"Premier");
			$pagination.= $this->getPaginationLink($counterstart - 1,"Précédent");
		}
		...
		if( $counterend != $maxPages ) {
			$pagination.= $this->getPaginationLink($counterend + 1,"Suivant");
			$pagination.= $this->getPaginationLink($maxPages,"Dernier");
		}
	...
	public function assignSimpleSearch() {
		...
		$this->xt->assign('searchbutton_attrs', 'id="searchButtTop'.$this->id.'" title="'."Rechercher".'"');
...
//		$settings["requestDelay"] = $option["forceExpire"] && $option["lifeTime"] < 2 ? 30 : 60;
	...
	*/
	...
	protected function getFieldFilterStateEntry( $fieldName, $value, $selectedValuesSet ) {
		...
		$displayValue = $displayValue !== "" ? $displayValue : "(" . "Vide" . ")";
