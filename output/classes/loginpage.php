...
class LoginPage extends RunnerPage
{
	...
	protected function prepareTwoFactorMessage() {
		...
		if( $destination["method"] == TWOFACTOR_PHONE ) {
			$twofMessage = str_replace( "%phone%", $destination["address"], "Un message texte avec votre code a été envoyé à: %phone%" );
		} else if( $destination["method"] == TWOFACTOR_EMAIL ) {
			$twofMessage = str_replace( "%email%", $destination["address"], "Un e-mail avec votre code a été envoyé à %email%." );
			...
			$twofMessage = str_replace(
				...
				"Entrez le code de votre application d'authentification correspondant à %username% à %site%."
			);
			$this->hideItemType("resend_button");
		}
	...
	protected function sendTwoFactorCode()
	{
		...
		if( !$ret["success"] )
		{
			$this->message = "Erreur d'envoi"." ".$ret["message"];
	...
	protected function refineMessage()
	{
		if( $this->message == "expired" )
			$this->message = "Votre session a expiré." . " " . "Reconnectez-vous.";
		elseif( $this->message == "invalidlogin" )
			$this->message = "Ouverture session invalide";
	...
	protected function isLoginAccessAllowed()
	{
		...
		$this->message = mysprintf( "Accès refusé pour %s minutes", array($this->auditObj->LoginAccess()) );
	...
	protected function assignBody()
	{
		...
		$this->body["end"] .= "<script type=\"text/javascript\" src=\"".GetRootPathForResources("include/runnerJS/RunnerAll.js?41053")."\"></script>";
