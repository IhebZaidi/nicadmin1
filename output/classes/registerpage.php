...
class RegisterPage extends RunnerPage
{
	...
	protected function activateNewUser()
	{
		...
		if( !$rs ) {
			echo "Code de validation non valable";
			return;
		}
	...
	protected function checkRegisterData( $strUsername, $strPassword, $strEmail )
	{
		...
		{
			$this->jsSettings['tableSettings'][ $this->tName ]['msg_userError'] = "Le nom d'utilisateur ne peut être vide";
			$ret = false;
		}
		else if( !$this->checkIfUsernameUnique( $strUsername ) )
		{
			$this->jsSettings['tableSettings'][ $this->tName ]['msg_userError'] = "Nom d'utilisateur"." <i>".runner_htmlspecialchars( $strUsername )."</i> "."existe deja. Choisir un autre nom d'utilisateur.";
			$ret = false;
		}
		...
		if( Security::emailField() && $this->pSet->appearOnPage( Security::emailField() ) )
		{
			...
			if( !strlen($strEmail) )
			{
				$this->jsSettings['tableSettings'][ $this->tName ]['msg_emailError'] = "Veuillez entrer une adresse courriel valide";
				$ret = false;
			}
			else if( !$this->checkIfEmailUnique( $strEmail ) )
			{
				$this->jsSettings['tableSettings'][ $this->tName ]['msg_emailError'] = "Courriel"." <i>". runner_htmlspecialchars( $strEmail )."</i> "."déjà enregistré. Si vous avez oublié votre nom d'utilisateur ou votre mot de passe utilisez la page retenir mot de passe";
				$ret = false;
			}
		}
	...
	protected function getPwdStrongFailedMessage()
	{
		...
		if($pwdLen)
		{
			$fmt = "Le mot de passe doit être composé de minimum %% caractères";
		...
		if($pwdUnique)
		{
			$fmt = "Le mot de passe doit être composé de minimum %% caractères distincts";
		...
		if($pwdDigits)
		{
			$fmt = "Le mot de passe doit être composé de minimum %% caractères ou symboles";
			$fmt = str_replace("%%", "".$pwdDigits, $fmt);
			$msg.= "<br>".$fmt;
		}
		if(GetGlobalData("pwdUpperLower", false))
		{
			$fmt = "Le mot de passe doit être composé de majuscules et minuscules";
			$msg.= "<br>".$fmt;
		}
